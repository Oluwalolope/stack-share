...
const workflows:Workflow[] = [
  {for: "WRITER",
    ...
    about: "A seamless bridge to convert long-form YouTube transcripts into 5 unique LinkedIn posts using custom GPT prompts and scheduled via Typefully.",
    likes: 1525000,
    views: 1200,
    tools: ["ChatGPT", "Framer", "Linkedin", "Bolt"],
    createdAt: "2h"
  },
  {for: "WRITER",
    by: "content_pro",
...
export default function Explore() {
  ...
  return (
    <div
      className={`h-screen bg-background ${spaceGrotesk.variable}`}
    >
      <Navbar />
...
<div className="flex gap-2">
          ...
          <div className="flex flex-col gap-10 w-full">
            ...
            {workflows.map((workflow, i) => (
                          <div key={i} className="flex flex-col lg:flex-row justify-between relative gap-6 p-5 border border-[#1E293B] rounded-xl ">
                          <div className="flex flex-col gap-2">
                            <div className="gap-3 flex">
                              ...
                              <p>@{workflow.by} • {workflow.createdAt} ago</p>
                            </div>
                            <h3 className="text-[#F1F5F9] text-xl font-bold cursor-pointer">{workflow.name}</h3>
                          ...
                          <div className=" flex flex-row lg:flex-col items-end justify-between border-t lg:border-t-0 lg:border-l border-[#1E293B] w-full lg:w-35 pt-2 lg:pt-0">
                            <div className=" flex flex-row lg:flex-col justify-between gap-4 lg:gap-1">
                              <div className="flex gap-1.5 items-center cursor-pointer text-sm font-medium">
                                <Image src={views} className="w-4" alt="views" /> {workflow.views > 999999? `${(workflow.views/1000000).toFixed(1)}M` : workflow.views > 999 ? `${(workflow.views/1000).toFixed(1)}K` : workflow.views}
                              </div>
                              <div className="text-[#0D93F2] flex gap-1.5 items-center cursor-pointer text-sm font-bold">
                                <Image src={likes} className="w-4" alt="likes" /> {workflow.likes > 999999? `${(workflow.likes/1000000).toFixed(1)}M` : workflow.likes > 999 ? `${(workflow.likes/1000).toFixed(1)}K` : workflow.likes}
                              </div>
                            </div>
            
                            <span className="p-2 place-content-center bg-[#1E293B] hover:bg-transparent rounded-lg cursor-pointer"><Image src={enterArrow} alt="selectArrow" /></span>
                          </div>
                          <div className="absolute w-full gap-2 hidden">
                            <span>
                              <Image src={RobloxIcon} alt="Tools Icons" />
                            </span>
                            <p>+2 more tools</p>
                          </div>
                        </div>
